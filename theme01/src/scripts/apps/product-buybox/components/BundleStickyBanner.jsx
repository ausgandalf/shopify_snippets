...
const BundleStickyBanner = () => {
  ...
  return (
    <div
      className={`sticky-banner fixed left-0 w-full py-4 bg-white transition-all duration-500 z-[100] border-b border-b-color-3 ${
        showBanner ? 'translate-y-0' : '-translate-y-full'
      }`}
    >
      <div className="container flex justify-between items-center">
        <div className="flex max-lg:flex-col lg:items-center lg:gap-24">
          <h6 className="text-grey-8">{product.title}</h6>

          <div className="max-lg:hidden">
            {product.options.map((optionGroup, groupIndex) => (
              <div className="flex items-center" key={`option-group-${groupIndex}`}>
                <span className="text-xs font-bold text-grey-8">{optionGroup.option.name.replace('(', '').replace(')', '')}:</span>
                <span className="ml-1 text-xs text-grey-8">{selectedVariant.options[optionGroup.option.position - 1]}</span>
              </div>
            ))}
          </div>
        </div>
...
export default BundleStickyBanner;
